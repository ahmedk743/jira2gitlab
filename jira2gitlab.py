...
def process_jira_issues_batch(batch_start_at, jira_project, gitlab_project_id, gl_milestones):
    total_issues_count = None
    try:
        batch_number = math.floor(batch_start_at / int(JIRA_PAGINATION_SIZE)) + 1
        ...
        try:
            ...
            total_issues_count = response_json.get('total')
            batch_issues = response_json.get('issues')
            log_frame_info(logger, f"Processing Jira issues batch: {batch_number}, "
                                   f"total issues: {total_issues_count}")
        except Exception as e:
            exception_handler(f"\nSkipping batch number: {batch_number}, "
                              f"issues: {batch_start_at}-{batch_start_at + int(JIRA_PAGINATION_SIZE)}"
                              f"\nUnable to query {query} in Jira!\n{e}")

        # Import issues into Gitlab
        if batch_issues:
            index_issues_map = [dict(index=batch_start_at + index, issue=issue)
                                for index, issue in enumerate(batch_issues, start=1)]
...
def process_jira_issues(jira_project, gitlab_project_id, gl_milestones):
    try:
        ...
        if total_issues_count is not None:
            ...
            log_frame_info(logger, f"Jira issues batch requests starting position: {batches_starting_positions}")

            run_parallel_with_generic_args(
                process_jira_issues_batch,
                iterable=batches_starting_positions,
                max_workers=jira_batch_max_workers,
                jira_project=jira_project,
                gitlab_project_id=gitlab_project_id,
                gl_milestones=gl_milestones,
            )
        succeeded_count = len([status for status in issue_process_statuses if status])
        failed_count = len(issue_process_statuses) - succeeded_count
        log_frame_info(logger, f"Jira issues processing complete, succeeded: {succeeded_count}, failed: {failed_count}")
...
def process_jira_issue(index_issue, gitlab_project_id, total_count, gl_milestones):
    ...
    log_frame_info(logger, f"Processing Jira issue #{index}, {issue_key}")
    ...
    try:
        ...
        store_import_status()
        log_frame_info(logger, f"Processed Jira issue #{index}, {issue_key}")
        issue_process_statuses.append(True)
    except Exception as e:
